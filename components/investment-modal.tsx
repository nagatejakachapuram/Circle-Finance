...
export function InvestmentModal({ open, onOpenChange, asset, investmentType, recipientAddress }: InvestmentModalProps) {
  ...
  // 1. Initialize state as an empty string
  ...
  // 2. Safely get chain info, handling the initially empty state
  const selectedChainInfo = selectedChain ? CCTP_CONFIG.chains[Number(selectedChain)] : null

  // This handler now correctly sets the string value from the Select component
  const handleChainChange = (value: string) => {
    console.log("[v1] Network selection changed to", value)
  ...
  const handleInvestment = async () => {
    ...
    try {
      console.log("[v1] Creating payment intent for multi-chain investment...")
      
      // 3. Convert selectedChain string to a number for the API call
      ...
      console.log("[v1] Processing payment on chain:", numericChainId)
      ...
      if (result.requiresWalletSignature && result.transactionData) {
        console.log("[v1] Requesting wallet transaction approval...")
        ...
        console.log("[v1] Transaction approved by user:", txHash)
      ...
      console.error("[v1] Investment transaction failed:", error)
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md bg-white border-slate-200">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Wallet className="w-5 h-5 text-[#3A86FF]" />Wallet Signature Required</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <Alert className="border-blue-200 bg-blue-50"><Wallet className="h-4 w-4 text-blue-600" /><AlertDescription className="text-blue-800">Please check your wallet and sign the transaction to transfer {totalCost.toFixed(2)} USDC for your investment in {asset.name}.</AlertDescription></Alert>
            <div className="p-4 bg-slate-50 rounded-lg">
              <div className="flex justify-between items-center mb-2"><span className="text-sm text-muted-foreground">Amount</span><span className="font-medium">{totalCost.toFixed(2)} USDC</span></div>
              <div className="flex justify-between items-center mb-2"><span className="text-sm text-muted-foreground">Network</span><span className="font-medium">{selectedChainInfo?.name || "Unknown"}</span></div>
              <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Tokens</span><span className="font-medium">{tokenAmount} {asset.name}</span></div>
            </div>
            <div className="flex items-center justify-center py-4"><Loader2 className="h-8 w-8 animate-spin text-[#3A86FF]" /></div>
  ...
  if (showResult && paymentResult) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md bg-white border-slate-200">
          <DialogHeader><DialogTitle className="flex items-center gap-2">{paymentResult.success ? <><CheckCircle className="w-5 h-5 text-green-600" />Investment Successful</> : <><XCircle className="w-5 h-5 text-red-600" />Investment Failed</>}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            {paymentResult.success ? (
              <div className="space-y-3">
                <Alert className="border-green-200 bg-green-50"><CheckCircle className="h-4 w-4 text-green-600" /><AlertDescription className="text-green-800">Your investment of {tokenAmount} tokens ({totalCost.toFixed(2)} USDC) in {asset.name} has been processed successfully on {selectedChainInfo?.name}!</AlertDescription></Alert>
                {(paymentResult.data?.txHash || paymentResult.txHash) && (<div className="p-3 bg-slate-50 rounded-lg"><p className="text-sm text-muted-foreground">Transaction Hash:</p><p className="font-mono text-xs break-all">{paymentResult.data?.txHash || paymentResult.txHash}</p></div>)}
              </div>
            ) : (
              <Alert className="border-red-200 bg-red-50"><XCircle className="h-4 w-4 text-red-600" /><AlertDescription className="text-red-800">{paymentResult.error || `Payment processing failed. Please ensure you have sufficient USDC balance on ${selectedChainInfo?.name}.`}</AlertDescription></Alert>
            )}
            <div className="flex gap-3">
              {paymentResult.success ? <Button className="flex-1 bg-gradient-to-tr from-[#3A86FF] to-[#1f6fff] text-white" onClick={() => onOpenChange(false)}>Done</Button> : (<><Button className="flex-1 bg-gradient-to-tr from-[#3A86FF] to-[#1f6fff] text-white" onClick={resetModal}>Try Again</Button><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button></>)}
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <DialogContent className="max-w-md bg-white border-slate-200">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {/* 4. Correctly structured Select component */}
          ...
          {!walletAddress && (<Alert className="border-yellow-200 bg-yellow-50"><AlertDescription className="text-yellow-800">Please connect your wallet to proceed with the investment.</AlertDescription></Alert>)}

          <Alert className="border-blue-200 bg-blue-50">
            <Wallet className="h-4 w-4 text-blue-600" />
            <AlertDescription className="text-blue-800">
                Ensure you have sufficient USDC balance on {selectedChainInfo?.name || "the selected network"} to complete this transaction.
            </AlertDescription>
          </Alert>
          ...
          <div className="flex gap-3 pt-4">
            <Button
              ...
              // 5. Disable button if no wallet OR no chain is selected
              disabled={isProcessing || !walletAddress || !selectedChain || (asset.minimumInvestment && totalCost < asset.minimumInvestment)}
            >
              {isProcessing ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Processing Payment...</>) : ("Proceed to Payment")}
            </Button>
